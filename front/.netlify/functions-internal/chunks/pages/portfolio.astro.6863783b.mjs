...
import 'html-escaper';
import { C as ChevronDownIcon, M as ModalGallery, S as Section, B as Button, j as AntiClockwiseIcon, G as GalleryGrid, g as getLangFromUrl, i as fetchImagesData, f as fetchGlobalData, P as PageWrapper, c as cave, $ as $$Layout } from './_...artistSlug_.astro.28d2e717.mjs';
...
function PortfolioPage({
  ...
  const f = {};
  globalFiltersData.filters.forEach((item) => f[item.title[language]] = "");
...
const $$Portfolio$2 = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { sectionNames, buttons, formData, layoutData, filtersData } = globalData;
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "image": cave }, { "default": ($$result2) => renderTemplate`
    ${renderComponent($$result2, "PageWrapper", PageWrapper, { "title": layoutData.navList[language][1].text, "main": layoutData.navList[language][0].text }, { "default": ($$result3) => renderTemplate`
        ${renderComponent($$result3, "PortfolioPage", PortfolioPage, { "client:load": true, "formTitle": sectionNames[language].form, "formData": formData[language], "button": cta, "globalFiltersData": filtersData, "fetchedData": imagesData, "language": language, "client:component-hydration": "load", "client:component-path": "pageComponents/PortfolioPage/PortfolioPage", "client:component-export": "PortfolioPage" })}
    ` })}
` })}`;
